# Running and stopping ZEBRALINK

Use a dedicated account portfolio. ZEBRALINK is an experiment capable of losing its entire allocated balance. The funding cap is **100 USDC at initialization**, not an assertion that USDC always equals one dollar.

## Installation and data

Python 3.11 or newer. `python -m zebralink prepare` downloads about 260 MB of public ZAPBench chunks and verifies them against `zebralink/neural/sources.lock.json`; `python -m zebralink verify` re-checks. `python -m zebralink train` fits the model (about a minute, 300 MB of RAM). Set `ZEBRALINK_DATA` to use another data location.

## Paper modes

```sh
# Real public prices; simulated fills and 0.6% fee per side.
python -m zebralink run --steps 10

# Explicit synthetic offline market, accelerated development run.
python -m zebralink run --fixture --fast --steps 10 --out runs/fixture

# Frozen-memory control, always in a separate run directory.
python -m zebralink run --fixture --fast --frozen --steps 10 --out runs/frozen
```
...
4. `pip install -e '.[live]'`. Copy `.env.example` to `.env`, set the key path and `COINBASE_PORTFOLIO_ID`, then set `ZEBRALINK_LIVE=I_ACCEPT_REAL_TRADES`. The CLI also requires `--live`.
5. `python -m zebralink run --live --preflight-only` reads permissions, balances and order state and initializes the ledger without submitting orders. Then `python -m zebralink run --live`.
